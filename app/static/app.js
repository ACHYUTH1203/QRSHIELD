...
function toggleScanner() {
    ...
    if (html5QrcodeScanner) {
        // Stop the scanner if it's currently running
        html5QrcodeScanner.stop().then(() => {
            html5QrcodeScanner = null;
            readerContainer.classList.add("hidden");
            btnCamera.innerHTML = `<i class="fa-solid fa-video"></i> Start Camera`;
            ...
            btnCamera.style.background = "#FFFFFF";
            btnCamera.style.color = "#111827";
            btnCamera.style.border = "1px solid #E5E7EB";
        });
    } else {
        // Unhide the container and update button styles
        readerContainer.classList.remove("hidden");
        btnCamera.innerHTML = `<i class="fa-solid fa-video-slash"></i> Close Camera`;
        btnCamera.style.background = "#FEE2E2"; 
        btnCamera.style.color = "#B91C1C";
        ...
        // Initialize and start the camera (THIS WAS MISSING)
        html5QrcodeScanner = new Html5Qrcode("reader");
        html5QrcodeScanner.start(
            { facingMode: { ideal: "environment" } }, // Fallback to webcam if no rear camera exists
            { fps: 12, qrbox: 260 },
            (decodedText) => {
                // When a QR code is successfully scanned:
                toggleScanner(); // Close the camera
                analyzePayload(decodedText); // Send payload to backend
            },
            (errorMessage) => {
            ...
            toggleScanner(); // Reset UI if camera fails to load
        });
    }
}